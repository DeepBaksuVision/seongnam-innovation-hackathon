...
from albumentations import (
    ShiftScaleRotate, Compose, GridDistortion, OpticalDistortion, Blur, GaussNoise, RandomBrightness, RandomContrast,
    RandomGamma, OneOf ,Resize
)
...
class Dataset(object):
    def __init__(self, root, transforms, train=True):
        ...
        self.select_class = ['bollard', 'bus', 'car', 'motorcycle', 'movable_signage', 'person',
                             'pole', 'potted_plant', 'traffic_light', 'traffic_sign',
    ...
    def __getitem__(self, idx):
        ...
        labels = self.labels[(self.labels['filename'] == self.imgs[idx].split("/sample_data/")[-1])]
        num_objs = labels.shape[0]
        boxes = []
        
        xmins, xmaxs, ymins, ymaxs, classes = labels['xmin'], labels['xmax'], labels['ymin'], labels['ymax'], labels['class']
        obj_class = []
    ...
    def _select_class(self):        
        ...
        self.imgs = [img for img in self.imgs if img.split("/sample_data/")[-1] in list(self.labels['filename'])]
...
def get_aug(p=0.5):
    return Compose([
        ...
        OneOf([
            OpticalDistortion(distort_limit=0.2, shift_limit=0, border_mode=cv2.BORDER_CONSTANT, value=0, p=0.2),
...
class Transform:
    ...
    def __call__(self, image, target):
        ...
        new_boxes = []
        for b in target:
            new_boxes.append([b[0] * x_ratio, b[1] * y_ratio, b[2] * x_ratio, b[3] * y_ratio])
